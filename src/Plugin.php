...
class Plugin {
	...
	public function frontend_scripts() {
		...
		if ( apply_filters( 'listings_chosen_enabled', true ) ) {
			...
			wp_register_script( 'listings-term-multiselect', LISTINGS_PLUGIN_URL . '/assets/js/term-multiselect.min.js', array(
				'jquery',
				'chosen'
			), LISTINGS_VERSION, true );
			wp_register_script( 'listings-multiselect', LISTINGS_PLUGIN_URL . '/assets/js/multiselect.min.js', array(
				'jquery',
				'chosen'
			), LISTINGS_VERSION, true );
		...
		if ( apply_filters( 'listings_ajax_file_upload_enabled', true ) ) {
			...
			wp_register_script( 'jquery-fileupload', LISTINGS_PLUGIN_URL . '/assets/js/jquery-fileupload/jquery.fileupload.js', array(
				'jquery',
				'jquery-iframe-transport',
				'jquery-ui-widget'
			), '9.11.2', true );
			wp_register_script( 'listings-ajax-file-upload', LISTINGS_PLUGIN_URL . '/assets/js/ajax-file-upload.min.js', array(
				'jquery',
				'jquery-fileupload'
			), LISTINGS_VERSION, true );

			ob_start();
			listings_get_template( 'form-fields/uploaded-file-html.php', array(
				'name'      => '',
				'value'     => '',
				'extension' => 'jpg'
			) );

<?php
/**
 * @todo: Docblocks: Going to need great in-code documentation to open this up to 3rd-party development.
 */
...
class Template {
	...
	public function get_template( $template_name, $args = array(), $template_path = 'listings' ) {
		if ( $args && is_array( $args ) ) {
			extract( $args );
			/** @todo: Extract won't pass WP VIP. We'll need to use the keys */
		}
	...
	public function get_template_part( $slug, $name = '', $template_path = 'listings' ) {
		...
		/**
		 * @todo: As a theme dev, I would love to have docblocks like this everywhere, with anything salient to theme
		 *      development.
		 */
		if ( ! $template ) {

...
class Shortcodes {
	...
	public function output_listings( $atts ) {
		...
		/** @todo Extract isn't allowed in WP VIP */
		...
		/**
		 * @todo: I think it's worth asking a few questions here about possible scenarios for themers. E.g., if I
		 *      wanted to add a button beside the "Show more" that said "Show less", how would I do that here? If I
		 *      needed to add a class to the load-more link for some random JS library, how could I do that?
		 */
		if ( $listings->have_posts() ) : ?>
			...
			<?php if ( $listings->found_posts > $per_page && $show_more ) : ?>
				...
				<?php else : ?>
					<a class="load_more_listings"
					   href="#"><strong><?php _e( 'Load more listings', 'listings' ); ?></strong></a>
					<?php
					/**
					 * @todo: Change class name to hyphenated from underscores (css best practices). Maybe, in addition
					 *      to a localization function, we should add a filter to the text here.
					 */
					?>
				<?php endif; ?>

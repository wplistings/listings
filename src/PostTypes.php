...
class PostTypes {
    ...
    public function job_feed() {
        ...
        if ( ! empty( $_GET['listing_categories'] ) ) {
            $cats     = explode( ',', sanitize_text_field( $_GET['listing_categories'] ) ) + array( 0 );
        ...
        query_posts( apply_filters( 'listing_feed_args', $query_args ) );
        add_action( 'rss2_ns', array( $this, 'listing_feed_namespace' ) );
        add_action( 'rss2_item', array( $this, 'listing_feed_item' ) );
        do_feed_rss2( false );
    }
     ...
     */
    public function listing_feed_namespace() {
        echo 'xmlns:listing="' .  site_url() . '"' . "\n";
    }
